...
The Google Search side of things asks Google Ads Keyword Planner how often keywords are searched. The API is free to use but you do need a Google Ads account and API access that Google approves.
...
4. Apply for "Basic Access" to the Google Ads API. Until it is granted, the API only answers for test accounts and returns no real keyword data.
...
- Refresh token stops working after 7 days – the OAuth consent screen is `External` + `Testing`. Set the user type to `External` (Workspace) or publish the app, then log in again.
